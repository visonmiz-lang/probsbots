...
import { TradingHistory } from "../types/trading-history";
...
- Learn from historical trading performance and avoid repeating mistakes
...
export function generateMultiSymbolPrompt(
  options: MultiSymbolPromptOptions, 
  ...
  const historySection = tradingHistory.isEmpty ? 
    `TRADING HISTORY: No historical trading data available. This is your first session or no completed trades yet.` :
    `
TRADING HISTORY ANALYSIS:

OVERALL STATISTICS (Last 30 days):
- Total Trades: ${tradingHistory.stats?.total_trades || 0}
- Win Rate: ${tradingHistory.stats?.win_rate || 0}%
- Avg Profit: +${tradingHistory.stats?.avg_profit_percent || 0}% | Avg Loss: ${tradingHistory.stats?.avg_loss_percent || 0}%
- Most Effective TP/SL: ${tradingHistory.stats?.avg_tp_percent || 0}% / ${tradingHistory.stats?.avg_sl_percent || 0}%

LEARN FROM MISTAKES - ANTI-PATTERNS:${tradingHistory.antiPatterns.length === 0 ? ' No clear anti-patterns identified yet.' : ''}
${tradingHistory.antiPatterns.map(pattern => 
  `❌ ${pattern.setup_pattern}: ${pattern.trade_count} trades, avg PnL ${pattern.avg_pnl}% (${pattern.losses} losses)`
).join('\n')}

RECENT PERFORMANCE:${tradingHistory.recentTrades.length === 0 ? ' No recent completed trades.' : ''}
${tradingHistory.recentTrades.map(trade => 
  `${trade.positionOutcome === 'win' ? '✅' : '❌'} ${trade.operation} | ${trade.leverage}x | SL:${trade.sl_percent}% TP:${trade.tp_percent}% | PnL: ${trade.pnl_percent}% | RSI:${trade.rsi_at_open} MACD:${trade.macd_at_open}`
).join('\n')}
`.trim();
...
CONSIDER HISTORICAL PERFORMANCE WHEN MAKING YOUR DECISION:
${tradingHistory.isEmpty ? 
  '- No historical data available yet - rely on current market analysis' : 
  '- Use the provided statistics to inform your risk management\n- Avoid repeating identified anti-patterns\n- Consider what has worked well in recent trades'
}
...
- Learn from historical trading patterns in the provided data
...
export function generateUserPrompt(
  ...
  const historySection = tradingHistory.isEmpty ? 
    `TRADING HISTORY: No historical trading data available yet.` :
    `
TRADING HISTORY:
- Total Trades: ${tradingHistory.stats?.total_trades || 0}
- Win Rate: ${tradingHistory.stats?.win_rate || 0}%
- Recent Performance: ${tradingHistory.recentTrades.slice(0, 3).map(trade => 
  `${trade.positionOutcome === 'win' ? '✅' : '❌'} ${trade.operation} PnL: ${trade.pnl_percent}%`
).join(', ')}
`.trim();
