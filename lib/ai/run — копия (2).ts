...
async function executeTradingDecision(decision: z.infer<typeof tradingSchema>, marketStates: any[]) {
  ...
  if ((operation === Opeartion.Buy || operation === Opeartion.Sell) && position) {
    ...
    // 🔥 ВЫВОДИМ ПОЛНЫЙ РЕЗУЛЬТАТ ОТ BYBIT
    console.log(`✅ FULL BYBIT TRADE RESULT:`, JSON.stringify(tradeResult, null, 2));

    // 🔥 ПОСЛЕ СОЗДАНИЯ ОРДЕРА ДЕЛАЕМ ЗАПРОС НА ПОЛУЧЕНИЕ ПОЗИЦИИ
...
export async function run(initialCapital: number) {
  ...
  try {
    ...
    // 🔥 ВЫВОДИМ ПОЛНЫЙ РЕЗУЛЬТАТ ТОРГОВЛИ ОТ BYBIT
    console.log(`✅ Trade execution completed with full Bybit response:`, tradeResult);
    console.log(`📊 Position data from Bybit:`, positionData);

    // 🔥 ЗАПИСЫВАЕМ В БД ДАННЫЕ ИЗ ПОЗИЦИИ (fetchpositions) - ТОЛЬКО ТЕ ПОЛЯ КОТОРЫЕ ЕСТЬ В СХЕМЕ
      ...
      data: {
        ...
        tradings: {
          create: {
            ...
            // 🔥 ЗАПИСЫВАЕМ ИЗ ПОЗИЦИИ: amount = size, pricing = entryPrice
    ...
    console.log("💾 Data saved to database with EXACT AI values and Bybit position data");
